...
- **中心化 Hooks**：
  ...
  - `useExtendedWeatherDataForOverlayChart`：扩展数据获取（用于风险叠加示意图）
- **地图图层 Hooks**：
...
数据层
    ...
    └── 扩展数据获取 (useExtendedWeatherDataForOverlayChart，用于风险叠加示意图)
```
...
### 6.6 扩展数据获取方案
**决策**：为风险叠加示意图单独获取扩展时间范围数据

**理由**：
- **计算准确性**：确保图表起始位置的数据点能正确回溯计算纵轴值
- **隔离性**：扩展数据仅用于风险叠加示意图，不影响其他模块
...
- **补充文档**：
  ...
  - `doc/补充-风险叠加示意图扩展数据获取方案.md`：扩展数据获取方案
