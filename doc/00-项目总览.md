...
### 4.2 数据计算模块
- **Mock数据生成器**：根据区域、天气类型和时间范围生成天气数据
- **保险产品库**：管理产品定义和产品级的风险事件触发条件
- **风险计算引擎**：基于产品规则计算风险事件
- **数据缓存**：使用useMemo优化性能
...
### 4.4 数据展示模块
- **分析汇总**：时间窗口、平均天气数值、总天气数值、风险事件数量、风险严重程度
- **图表可视化**：
  - 天气数值柱状图（按天/按小时）
  - 天气数据和风险叠加示意图（三种产品类型）
  - 风险事件触发时间线
...
Mock 数据生成/补充 (useMemo 缓存)
    ↓
风险计算引擎 (从产品库获取产品级的风险事件触发条件)
    ↓
风险事件计算 (基于产品级的风险事件触发条件)
    ↓
地图渲染更新 (图层更新)
    ↓
数据面板更新 (图表更新)
