...
**重要说明**：
- 本步骤定义**产品级的风险事件触发条件**（用于风险计算和可视化）
- 同时定义**保单级的赔付原则**（仅用于产品介绍页的教育展示，不参与计算）
- 产品库管理两层信息：产品级的风险事件触发条件用于计算，保单级的赔付原则用于展示
...
- 了解产品级规则的定义方式（详见需求文档 #4.3.2 产品级风险事件触发条件）

### 需要的数据/接口
- 产品基本信息：ID、名称、描述、图标
- **产品级规则**：触发条件、阈值配置、风险级别定义（tier 1, tier 2, tier 3）
- **保单级规则**（仅用于教育展示）：赔付频率限制、理赔额度百分比（tier 1: 20%, tier 2: 50%, tier 3: 100%）
...
1. **产品数据模型定义**
   ...
   - 定义`RiskRule`接口（产品级的风险事件触发规则，用于计算）
   - 定义`PayoutRule`接口（保单级的赔付原则，仅用于教育展示）
   - 定义`Threshold`接口（阈值配置，对应 tier 1, tier 2, tier 3）
   - 定义产品类型枚举（daily/weekly/monthly）
   - **注意**：`PayoutRule`为可选字段，仅用于产品介绍页展示，不参与风险计算
...
interface TimeWindowConfig {
  type: 'hourly' | 'daily' | 'weekly' | 'monthly'; // 时间单位类型
  size: number; // 窗口大小（单位由 type 决定）
  step?: number; // 滑动步长（可选，默认1，用于滑动窗口）
}
...
- [ ] 产品定义验证机制正常工作（如果实现）
- [ ] 产品数据结构与需求文档一致
- [ ] 产品数据模型包含产品级的风险事件触发条件（`riskRules`，用于计算）
- [ ] 产品数据模型包含保单级的赔付原则（`payoutRules`，可选，仅用于教育展示）
...
#### 5. 保单级信息支持（仅用于教育展示）
- **设计目标**：支持在产品配置中定义保单级的赔付原则，用于产品介绍页的教育展示
- **实现方式**：
  - `Product` 接口新增 `payoutRules?: PayoutRule` 可选字段
  - `PayoutRule` 接口包含：
    - `frequencyLimit?: string`：赔付频率限制（如"once per day per policy"）
    - `payoutPercentages: { tier1: number, tier2: number, tier3: number }`：理赔额度百分比（tier 1: 20%, tier 2: 50%, tier 3: 100%）
  - **重要说明**：`payoutRules` 为可选字段，仅用于产品介绍页展示，不参与风险计算引擎的计算
  - 风险计算引擎只使用 `riskRules`，完全忽略 `payoutRules`
...
- 支持自定义天气类型和单位
- 未来如需实现保单级计算，可直接使用 `payoutRules` 字段
