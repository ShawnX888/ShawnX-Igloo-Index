...
const allRegionsData = useMemo(() => {
  const cacheKey = `${selectedRegion.district}-${dateRange.from}-${dateRange.to}-${rainfallType}`;
  ...
  // 生成数据
  const data = generateRainfallData(selectedRegion, dateRange, rainfallType);
...
// 伪代码示例
// 使用React.memo
const RainfallChart = React.memo(({ data, viewMode }: Props) => {
  // 组件实现
}, (prevProps, nextProps) => {
  // 自定义比较逻辑
