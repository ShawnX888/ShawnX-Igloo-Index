# 扩展性设计：多天气类型支持

## 概述

为了支持未来扩展多种天气类型（不仅仅是降雨量）和相应的保险产品，我们对系统进行了扩展性改造。本文档说明改造内容和使用方法。

**文档版本**：v1.0  
**创建日期**：2025-01-27  
**最后更新**：2025-01-27

---

## 设计目标

1. **支持多种天气类型**：降雨量、温度、风速、湿度、气压、降雪等
2. **向后兼容**：现有代码（基于降雨量）无需修改即可继续工作
3. **类型安全**：使用 TypeScript 类型系统确保类型一致性
4. **易于扩展**：添加新的天气类型只需配置，无需修改核心逻辑

---

## 类型系统扩展

### 1. 天气类型定义

```typescript
// src/types/data.ts
export type WeatherType = 'rainfall' | 'temperature' | 'wind' | 'humidity' | 'pressure' | 'snowfall';
export type DataType = 'historical' | 'predicted';

// 向后兼容
export type RainfallType = DataType; // 'historical' | 'predicted'
```

### 2. 通用天气数据接口

```typescript
// 通用天气数据接口（推荐使用）
export interface WeatherData {
  date: string; // ISO格式
  value: number; // 数值（根据天气类型有不同的单位和含义）
  risk?: 'low' | 'medium' | 'high';
  weatherType?: WeatherType; // 天气类型标识
}

// 区域天气数据集合
export interface RegionWeatherData {
  [districtName: string]: WeatherData[];
}

// 向后兼容：降雨量数据接口
export interface RainfallData {
  date: string;
  amount: number; // 降雨量 (mm)
  risk?: 'low' | 'medium' | 'high';
}

export interface RegionData {
  [districtName: string]: RainfallData[];
}
```

### 3. 产品类型扩展

```typescript
// src/types/product.ts
export interface Product {
  id: string;
  name: string;
  type: ProductType; // 时间维度：'daily' | 'weekly' | 'monthly'
  weatherType: WeatherType; // 数据维度：'rainfall' | 'temperature' | ...
  description: string;
  icon: string;
  riskRules: RiskRule;
}

export interface RiskRule {
  triggerType: ProductType; // 时间维度
  weatherType: WeatherType; // 数据维度，必须与 Product.weatherType 一致
  timeWindow: TimeWindowConfig;
  thresholds: Threshold[];
  calculation: CalculationConfig;
}

export interface CalculationConfig {
  aggregation: 'sum' | 'average' | 'max' | 'min';
  operator: '>' | '<' | '>=' | '<=' | '==';
  unit: 'mm' | 'inch' | 'celsius' | 'fahrenheit' | 'kmh' | 'mph' | 'percent' | 'hpa' | 'psi';
}
```

---

## 数据生成器扩展

### 1. 通用天气数据生成器

**文件**：`src/lib/weatherDataGenerator.ts`

```typescript
export interface WeatherDataGenerator {
  generate(
    region: Region,
    dateRange: DateRange,
    dataType: DataType,
    weatherType: WeatherType
  ): RegionWeatherData;
  
  hasData(...): boolean;
  supplement(...): RegionWeatherData;
  getDailyData(...): WeatherData[];
}

export class MockWeatherDataGenerator implements WeatherDataGenerator {
  // 实现支持所有天气类型的数据生成
}
```

**特点**：
- 支持所有天气类型（通过 `WEATHER_CONFIGS` 配置）
- 每种天气类型有独立的生成参数（基础值范围、时间因子、季节性因子等）
- 自动计算风险级别（根据天气类型和数值）

### 2. 向后兼容的降雨量生成器

**文件**：`src/lib/rainfallDataGenerator.ts`

```typescript
// 向后兼容：内部使用 WeatherDataGenerator
export class MockRainfallDataGenerator implements RainfallDataGenerator {
  generate(region: Region, dateRange: DateRange, type: DataType): RegionData {
    // 内部调用 weatherDataGenerator.generate(..., 'rainfall')
    // 并转换为 RainfallData 格式
  }
}
```

**特点**：
- 保持现有接口不变
- 内部使用通用生成器
- 自动转换数据格式

---

## 产品库扩展

### 1. 产品配置

**文件**：`src/data/products.ts`

所有产品现在必须包含 `weatherType` 字段：

```typescript
{
  id: 'daily',
  name: 'Daily Heavy Rain',
  type: 'daily',
  weatherType: 'rainfall', // 新增字段
  description: '...',
  icon: '🌧️',
  riskRules: {
    triggerType: 'daily',
    weatherType: 'rainfall', // 必须与 Product.weatherType 一致
    // ...
  }
}
```

### 2. 产品查询扩展

未来可以扩展 `ProductLibrary` 支持按天气类型查询：

```typescript
class ProductLibrary {
  // 现有方法
  getProduct(id: string): Product | null;
  getProductsByType(type: ProductType): Product[];
  
  // 新增方法（未来实现）
  getProductsByWeatherType(weatherType: WeatherType): Product[];
  getProductsByTypeAndWeather(type: ProductType, weatherType: WeatherType): Product[];
  getSupportedWeatherTypes(): WeatherType[];
}
```

---

## 使用指南

### 1. 生成通用天气数据

```typescript
import { weatherDataGenerator } from '@/lib/weatherDataGenerator';

// 生成降雨量数据
const rainfallData = weatherDataGenerator.generate(
  region,
  dateRange,
  'historical',
  'rainfall'
);

// 生成温度数据
const temperatureData = weatherDataGenerator.generate(
  region,
  dateRange,
  'historical',
  'temperature'
);
```

### 2. 使用向后兼容的降雨量生成器

```typescript
import { rainfallDataGenerator } from '@/lib/rainfallDataGenerator';

// 现有代码无需修改
const rainfallData = rainfallDataGenerator.generate(
  region,
  dateRange,
  'historical'
);
```

### 3. 定义新产品

在 `src/data/products.ts` 中添加新产品：

```typescript
{
  id: 'temperature-daily',
  name: 'Daily High Temperature',
  type: 'daily',
  weatherType: 'temperature', // 指定天气类型
  description: 'Daily maximum temperature > threshold',
  icon: '🌡️',
  riskRules: {
    triggerType: 'daily',
    weatherType: 'temperature', // 必须一致
    timeWindow: { type: 'hourly', size: 24, step: 1 },
    thresholds: [
      { value: 35, level: 'high', label: '35°C' },
      { value: 30, level: 'medium', label: '30°C' },
      { value: 25, level: 'low', label: '25°C' }
    ],
    calculation: {
      aggregation: 'max',
      operator: '>',
      unit: 'celsius'
    }
  }
}
```

---

## 扩展新天气类型

### 1. 添加天气类型定义

在 `src/types/data.ts` 中扩展 `WeatherType`：

```typescript
export type WeatherType = 
  | 'rainfall' 
  | 'temperature' 
  | 'wind' 
  | 'humidity' 
  | 'pressure' 
  | 'snowfall'
  | 'newType'; // 新增类型
```

### 2. 添加生成配置

在 `src/lib/weatherDataGenerator.ts` 的 `WEATHER_CONFIGS` 中添加配置：

```typescript
const WEATHER_CONFIGS: Record<WeatherType, WeatherGenerationConfig> = {
  // ... 现有配置
  newType: {
    baseRange: { min: 0, max: 100 },
    historicalFactor: 1.0,
    predictedFactor: 0.9,
    timeFactors: [/* ... */],
    seasonalStrength: 0.3
  }
};
```

### 3. 添加风险级别计算逻辑

在 `calculateRiskLevel` 函数中添加新类型的风险计算：

```typescript
function calculateRiskLevel(weatherType: WeatherType, value: number): ... {
  switch (weatherType) {
    // ... 现有case
    case 'newType':
      // 定义风险级别计算逻辑
      if (value >= threshold) return 'high';
      // ...
      return undefined;
  }
}
```

### 4. 添加单位支持

在 `src/types/product.ts` 的 `CalculationConfig.unit` 中添加新单位（如果需要）：

```typescript
unit: 'mm' | 'inch' | 'celsius' | 'fahrenheit' | ... | 'newUnit';
```

---

## 向后兼容性

### 已保持兼容的部分

1. **类型别名**：`RainfallType = DataType`，现有代码无需修改
2. **接口保留**：`RainfallData` 和 `RegionData` 接口保留，现有代码继续工作
3. **生成器接口**：`RainfallDataGenerator` 接口保留，内部使用通用生成器
4. **组件使用**：现有组件（如 `ControlPanel`、`Dashboard`）无需修改

### 需要更新的部分（可选）

如果希望使用新的通用接口，可以逐步迁移：

1. 将 `RainfallType` 替换为 `DataType`
2. 将 `RainfallData` 替换为 `WeatherData`
3. 将 `RegionData` 替换为 `RegionWeatherData`
4. 使用 `WeatherDataGenerator` 替代 `RainfallDataGenerator`

---

## 相关文档

- `doc/04-Mock数据生成器.md` - 数据生成器详细说明（已更新）
- `doc/06-产品库基础结构.md` - 产品库详细说明（已更新）
- `src/types/data.ts` - 类型定义
- `src/lib/weatherDataGenerator.ts` - 通用天气数据生成器实现
- `src/data/products.ts` - 产品配置示例

---

## 总结

通过本次扩展性改造：

1. ✅ **类型系统**：支持多种天气类型，类型安全
2. ✅ **数据生成器**：通用生成器支持所有天气类型，向后兼容
3. ✅ **产品库**：产品支持天气类型字段，易于扩展
4. ✅ **文档**：更新了相关文档，说明扩展性设计
5. ✅ **向后兼容**：现有代码无需修改即可继续工作

未来添加新的天气类型和产品时，只需：
1. 在 `WeatherType` 中添加新类型
2. 在 `WEATHER_CONFIGS` 中添加配置
3. 在 `calculateRiskLevel` 中添加风险计算逻辑
4. 在 `products.ts` 中添加新产品定义

即可完成扩展，无需修改核心逻辑。

