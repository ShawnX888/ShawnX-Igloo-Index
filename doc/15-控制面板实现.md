...
### 步骤目标
实现手动选择控制面板，包含区域选择（模糊搜索、层级选择）、数据类型切换、时间范围选择、产品选择等功能，支持与AI对话面板的互斥显示。
...
- 产品数据：`Product[]`类型
- 当前选择状态：`selectedRegion`、`rainfallType`、`dateRange`、`selectedProduct`
  ...
  return (
    <div className="flex gap-2">
      <button
        ...
        className={rainfallType === 'historical' ? 'selected' : ''}
      >
        历史降雨量
      </button>
      <button
        onClick={() => handleTypeChange('predicted')}
        className={rainfallType === 'predicted' ? 'selected' : ''}
      >
        预测降雨量
      </button>
    </div>
  ...
  return (
    <div>
      ...
      <DatePicker
        from={dateRange.from}
        to={dateRange.to}
        maxDays={rainfallType === 'historical' ? 40 : 10}
