...
### 步骤目标
实现三种保险产品类型（降雨量日内产品、降雨量周度产品、降雨量月度产品）的完整配置，包括产品基本信息、风险触发规则、阈值配置和风险级别定义，完成产品库的完整功能实现。
  ...
  "products": [
    {
      ...
      "description": "连续4小时的降雨量超过阈值时触发（时间区间：00:00 to 23:59）",
      "icon": "cloud-rain",
      "riskRules": {
        "triggerType": "daily",
        "weatherType": "rainfall",
...
### 功能验收标准
- [ ] 降雨量日内产品配置完整，包含所有必要信息
- [ ] 降雨量周度产品配置完整，包含所有必要信息
- [ ] 降雨量月度产品配置完整，包含所有必要信息
- [ ] 所有产品的阈值配置正确（符合需求文档）
- [ ] 所有产品的天气类型标识正确（降雨量）
- [ ] 产品查询功能正常工作（按ID、按类型）
- [ ] 产品验证功能正常工作，可以检测配置错误
...
- 目前该网站仅可视化产品级的风险事件触发条件，不涉及保单级的赔付原则
