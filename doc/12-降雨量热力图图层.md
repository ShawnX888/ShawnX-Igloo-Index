...
### 步骤目标
实现历史降雨量和预测降雨量的热力图图层，使用Data Layer和Polygon实现"从中心往边界逐渐变浅"的渐变效果，支持图层切换和动态更新。
...
dataLayer.setStyle((feature) => {
  ...
  const rainfall = getRainfallData(district);
  
  return {
    fillColor: calculateColor(rainfall),
    fillOpacity: calculateOpacity(rainfall),
    strokeColor: '#4285F4',
    strokeWeight: 1,
  };
});
...
#### 2. 径向渐变效果
由于Google Maps Polygon不支持CSS渐变，需要使用以下方案：
- **方案A**：使用多个同心圆Polygon模拟渐变效果
- **方案B**：使用Canvas Overlay实现渐变效果
- **方案C**：使用预生成的渐变图片作为填充

#### 3. 颜色计算
```typescript
// 伪代码示例
function calculateColor(rainfall: number, type: RainfallType): string {
  const maxRainfall = 200; // 最大降雨量参考值
...
### 常见问题
1. **渐变效果难以实现**
   - Google Maps Polygon不支持CSS渐变
   - 需要使用替代方案（多个同心圆、Canvas Overlay等）
...
### 技术难点
- **渐变效果实现**：Google Maps不直接支持渐变，需要创造性解决方案
