...
function RiskEventMarkersLayer({
  ...
  useEffect(() => {
    ...
    // 计算每个区域的风险事件数量
    // 根据dataType过滤风险事件（历史或预测）
    const filteredEvents = riskEvents.filter(
      event => event.dataType === dataType
    );
    const regionCounts = calculateRegionCounts(regions, filteredEvents);
    ...
    const markers = regions.map(region => {
      ...
      const baseColor = dataType === 'historical' ? '#ef4444' : '#f97316';
  ...
  }, [map, regions, riskEvents, selectedRegion, dataType, selectedProduct]);
  
  // 注意：风险事件数据中的 dataType 字段用于区分历史/预测数据
  // weatherType 字段用于标识对应的天气类型（如 'rainfall'），对应产品的weatherType字段
