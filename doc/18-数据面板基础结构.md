...
2. **数据获取逻辑**
   - 从 Dashboard 组件获取当前参数
   - 根据参数（`dataType` 和 `weatherType`）获取对应区域的天气数据
   - 根据参数和产品获取风险事件数据
   - 使用 `useMemo` 缓存计算结果
...
export function DataDashboard({
  selectedRegion,
  dataType,      // 数据类型：'historical' | 'predicted'
  dateRange,
  selectedProduct,
  dailyData,     // 当前选定区域的天气数据
  onProductSelect
}: DataDashboardProps) {
  // 获取风险事件数据
  // 注意：计算引擎返回的 RiskEvent 包含 dataType 和 weatherType 字段
...
function useDashboardData(
  ...
  allRegionsData: RegionData // 区域数据集合：保持名称不变
) {
  // 从区域数据中获取当前选定行政区域的天气数据
  ...
  }, [selectedProduct, selectedRegion, dateRange, weatherData]);

  // 计算统计数据
  const statistics = useMemo(() => {
    return calculateStatistics(weatherData, riskEvents, selectedProduct);
  }, [weatherData, riskEvents, selectedProduct]);
...
useEffect(() => {
  // 参数变更时，自动重新获取数据
  // 数据更新时，自动更新面板显示
}, [selectedRegion, dataType, dateRange, selectedProduct]);
```
...
- **当前参数**：`selectedRegion`、`dataType`、`dateRange`、`selectedProduct`
- **天气数据**：`WeatherData[]` 类型
- **区域数据**：`RegionData` 类型（以区域为索引的完整数据集）
