...
### 步骤目标
实现产品库与风险计算引擎的协同机制，确保产品级的风险事件触发条件可以正确传递给计算引擎，计算结果可以正确返回，完成端到端的数据流转和计算流程验证。

**重要说明**：本步骤仅涉及**产品级的风险事件触发条件**的协同，不涉及保单级的赔付计算。产品库提供产品级的风险事件触发条件定义，计算引擎基于这些条件计算风险事件，不涉及赔付金额、赔付频率等保单级规则。

### 预期成果
- 产品库与计算引擎接口对接完成
- 产品级的风险事件触发条件可以正确传递给计算引擎
- 计算引擎可以正确解析产品级的风险事件触发规则并执行计算
- 计算结果可以正确返回和格式化
- 端到端的计算流程验证通过
...
1. **接口对接**
   - 定义产品库与计算引擎的标准接口
   - 实现产品级的风险事件触发条件定义的传递机制
   - 实现计算结果的返回机制
   - **注意**：仅传递产品级的风险事件触发条件，不涉及保单级的赔付原则

2. **数据流转验证**
   - 验证产品级的风险事件触发条件可以正确传递
   - 验证计算引擎可以正确解析产品级的风险事件触发规则
   - 验证计算结果可以正确返回
   - 验证数据格式的一致性
   - **注意**：仅验证产品级的风险事件触发条件，不涉及保单级的赔付原则

3. **计算流程验证**
   - 验证降雨量日内产品的计算流程（基于产品级的风险事件触发条件）
   - 验证降雨量周度产品的计算流程（基于产品级的风险事件触发条件）
   - 验证降雨量月度产品的计算流程（基于产品级的风险事件触发条件）
   - 验证不同阈值档位（tier 1, tier 2, tier 3）的计算逻辑
   - **注意**：仅验证产品级的风险事件触发条件计算，不涉及保单级的赔付计算
...
有关产品库与计算引擎协同的中心化 Hook 实现方案，请参阅：[风险计算架构升级方案](./补充-风险计算架构升级方案.md)
