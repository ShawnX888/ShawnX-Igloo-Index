...
2. **数据集成逻辑**
   - 从 Dashboard 组件接收通过 `useRiskAnalysis` 计算好的 `riskEvents` 和 `statistics` (风险分级统计)
   - 接收通过 `useWeatherStatistics` 计算好的 `weatherStatistics` (天气基础统计)
   - 气象原始数据 `dailyData` 同样通过 Props 传入
   - 组件内部仅处理与视图展示相关的逻辑

3. **数据联动机制**
   - 核心参数（区域、产品等）变更触发顶层 Hook (`useRiskAnalysis`, `useWeatherStatistics`) 重新计算
...
4. **数据格式化**
   ...
   - `weatherStatistics` 和 `statistics` (风险统计) 直接用于渲染顶层的分析汇总卡片

### 技术实现方案

有关中心化风险计算架构、天气统计解耦及数据流向的详细方案，请参阅：[风险计算架构升级方案](./补充-风险计算架构升级方案.md)

#### 1. 数据面板组件结构
```typescript
// 核心逻辑已上移至 Dashboard.tsx 的 Hook 层
export function DataDashboard({
  ...
  statistics,          // 风险分级统计信息
  weatherStatistics,   // 天气基础统计信息
  onNavigateToProduct
}: DataDashboardProps) {
  // 组件内不再执行核心计算，仅负责视图渲染和局部交互状态
  return (
