...
- **`useExtendedWeatherData.ts`** - 扩展数据获取 Hook（通用）
  - 为风险叠加示意图和风险事件计算获取扩展时间范围数据
  - 根据产品 `riskRules.timeWindow` 动态计算扩展范围
  - 确保图表起始位置能正确回溯计算纵轴值
  - 确保风险事件计算在时间窗口起始位置的准确性

**分析计算 Hooks**
- **`useRiskAnalysis.ts`** - 中心化风险分析 Hook（唯一事实来源）
  - 输入：区域、时间范围、产品、天气数据
  - 内部使用 `useExtendedWeatherData` 获取扩展时间范围的天气数据
  - 输出：风险事件列表（已过滤到用户选择的时间窗口）、地图标记数据、统计信息
  - 自动触发计算，带缓存机制
...
**使用场景**：
1. **风险叠加示意图**：使用扩展数据计算纵轴值，确保图表起始位置的数据点能正确回溯计算
2. **风险事件计算**：使用扩展数据确保时间窗口起始位置的风险事件计算准确
...
// useRiskAnalysis.ts (在风险分析 Hook 中)
const { extendedHourlyData, extendedDailyData } = 
  useExtendedWeatherData(
    selectedRegion,
    dateRange,
...
// useExtendedWeatherData.ts (Hook 层，通用)
