...
- 同时实现**保单级的赔付规则**配置（用于 v2 claim 理赔计算；前端也可用于产品介绍页展示）
- 产品配置包含两层信息：`riskRules`用于风险事件计算与可视化，`payoutRules`用于理赔计算
...
- **注意**：产品配置包含两层信息：
  ...
  - `payoutRules`：保单级的赔付规则（用于 v2 claim 理赔计算；不参与风险事件计算）
...
5. **保单级赔付规则配置**（用于 v2 claim 理赔计算）
   - 赔付频率限制：如"once per day per policy"、"once per month per policy"
   - 理赔额度百分比：tier 1: 20%, tier 2: 50%, tier 3: 100%（前端仅展示百分比数值）
   - 赔付金额计算规则：如“赔付金额 = 保额 × 对应档位百分比”
   - 总金额限制：如“总赔付不超过保额/或不超过固定上限”
   - **注意**：保单级赔付规则不参与风险事件计算（risk），用于 v2 claim 理赔计算；前端可同时用于产品介绍页展示
    ...
    },
    {
      ...
      "payoutRules": {
        ...
        "payoutAmount": {
          ...
          "formula": "payout = sumInsured * payoutPercentages[tier]"
        },
        "amountLimit": {
          "scope": "per policy",
          "type": "total_cap",
          "maxAmount": "sumInsured",
          "note": "Total payout is capped at the sum insured."
        }
      }
    },
    {
...
- `payoutRules`：保单级的赔付规则，用于 v2 claim 理赔计算（前端也可用于产品介绍页展示）；不参与风险事件计算
  - `frequencyLimit`：赔付频率限制（可选，如"once per day per policy"）
  - `payoutPercentages`：理赔额度百分比，对应 tier 1/2/3 分别为 20%、50%、100%（前端仅展示百分比数值）
  - `payoutAmount`：赔付金额计算规则（用于展示，例如“赔付金额 = 保额 × 对应档位百分比”）
  - `amountLimit`：总金额限制（用于展示，例如“总赔付不超过保额/固定上限”）
...
- [ ] 产品查询功能正常工作（按ID、按类型）
- [ ] 产品验证功能正常工作，可以检测配置错误
- [ ] 风险计算引擎仅使用 `riskRules`（风险事件计算与可视化）
- [ ] v2 claim 理赔计算使用 `payoutRules`（赔付频率/金额/上限等规则）
...
- 支持产品配置的热更新（未来扩展）
- **保单级信息说明**：
  - `payoutRules` 用于 v2 claim 理赔计算（赔付频率、赔付金额、总额上限等），前端也可用于产品介绍页展示
  - 风险计算引擎（risk）完全忽略 `payoutRules`，只使用 `riskRules` 进行风险事件计算
  - 理赔额度百分比（20%、50%、100%）前端可以展示百分比数值；理赔计算引擎可基于该百分比结合 `payoutAmount`/`amountLimit` 计算金额
...
- **产品级 vs 保单级**：
  ...
  - **保单级的赔付规则**（v2 claim 使用）：定义赔付多少金额，赔付几次，以及是否存在总额上限（如"once per day per policy"、"对应不同理赔额度"、"total cap"）
- **当前网站范围**：风险模块仅计算并可视化产品级的风险事件触发条件；v2 claim 模块基于 `payoutRules` 进行保单级理赔计算
