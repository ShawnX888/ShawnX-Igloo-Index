...
- `calculation_run_id`：计算运行ID（用于区分同一时间点的不同计算批次）
  - **用途**：区分历史数据（不可变）和预测数据（易变）
  - **历史数据**：calculation_run_id为固定值或NULL，数据不可变
  - **预测数据**：每次重新计算预测时，生成新的calculation_run_id，标记旧预测失效或覆盖旧数据

**唯一约束**：同一产品、区域、时间、天气类型、tier级别、calculation_run_id只能有一条记录

**数据流差异**：
- **历史风险事件**：数据不可变，一旦计算完成即固定
- **预测风险事件**：数据易变，每次预测数据更新时重新计算
  - 策略1：覆盖旧预测（删除旧calculation_run_id的记录，插入新记录）
  - 策略2：标记旧预测失效（保留历史记录，新增记录标记为active）
...
- `GET /risk-events/calculate` - 实时计算风险事件（触发计算，结果通过POST接口保存）
  - **异步处理**：如果计算超过0.5秒，建议通过Webhook或Polling机制异步处理
  - **实现方式**：使用Celery异步任务，或使用run_in_executor将计算扔到线程池
...
**实现示例**：
- `GET /risk-events/calculate`：如果计算超过0.5秒，返回任务ID，客户端通过Polling获取结果
- `POST /claims/calculate`：使用Celery异步任务，通过Webhook通知结果
...
**粒子效果库**（使用deck.gl）：
- `rainParticles.ts`：降雨粒子效果（使用deck.gl的ParticleLayer）
- `windParticles.ts`：风速粒子效果（使用deck.gl的ParticleLayer）
- `typhoonParticles.ts`：台风粒子效果（未来扩展，使用deck.gl）
...
**技术实现（推荐方案）**：
- **deck.gl**：推荐使用deck.gl（Uber开源）而非Three.js
  - 原因：deck.gl专门为大规模地理数据可视化设计，对粒子流、风场等有现成的ParticleLayer
  - 优势：高性能、稳定、完美集成Google Maps，比手写Three.js粒子系统更快且更稳定
  - 适用场景：大规模粒子效果、风场可视化、台风轨迹等
- **Three.js（备选）**：仅在需要复杂3D效果时考虑
  - 风险：Google Maps上叠加Three.js Canvas对性能要求极高，尤其是移动端
  - MVP建议：不要一开始就追求3D粒子，优先使用deck.gl

**性能优化**：
- 使用WebGL进行高性能渲染
- 根据地图缩放级别动态调整粒子密度
- 移动端降级策略：降低粒子数量或使用简化效果
...
7. 实现calculation_run_id机制，区分历史数据和预测数据
...
- v2.1 (2025-01-27)：
  ...
  - 新增风险事件表的calculation_run_id字段，区分历史数据和预测数据
  - 完善技术风险和注意事项章节
