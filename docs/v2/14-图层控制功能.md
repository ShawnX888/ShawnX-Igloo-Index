...
### 步骤目标
实现图层控制UI，支持开启/关闭各个数据图层（历史/预测天气数据热力图、风险事件标记），根据数据类型和产品选择动态显示可操作的图层，UI设计参考Google Maps。
...
3. **动态显示逻辑**
   - 历史数据模式：仅"Historical Weather"可操作
   - 预测数据模式：仅"Predicted Weather"可操作
   - 风险事件图层：仅在选中产品时可用
   - 不可操作的图层显示为禁用状态
    ...
    <Popover>
      ...
      <PopoverContent>
        ...
        <LayerToggle
          ...
          enabled={dataType === 'historical'}
          checked={layers.heatmap && dataType === 'historical'}
          onChange={(checked) => {
        ...
        <LayerToggle
          ...
          enabled={dataType === 'predicted'}
          checked={layers.heatmap && dataType === 'predicted'}
          onChange={(checked) => {
