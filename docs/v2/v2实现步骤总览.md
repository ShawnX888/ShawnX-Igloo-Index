...
| 21 | L0 Sidebar（省级态势抽屉） | `docs/v2/v2实施细则/21-L0-Sidebar省级态势抽屉-细则.md` |
...
| 39 | AI Insight Cards | `docs/v2/v2实施细则/39-AI-Insight-Cards-细则.md` |
...
每个里程碑至少跑通一次：
```
L0（省级态势） → 锁区（Map Click / Ranking Click） → L1（区域情报） →（按 Mode）L2（证据链）
```
...
| **Lane B: Backend** | products/policies/claims/risk_events/prediction_runs + Data Products + Tasks | API + 服务 + 任务 |
| **Lane C: Frontend** | UI Orchestration + Map Stage + L0/L1/L2 UI | 前端组件 + 状态管理 |
| **Lane D: Governance** | 合规 gate、观测、发布治理 | 配置 + 监控 + 审计 |
| **Lane E: AI** | 聊天窗、洞察卡、Intent 执行链路 | Agent + Tool Calling + CTA 联动 |
...
| 06 | 保单表 + Policy Service | B | 05 | R2 | policies 表 + API | Data Product | timezone 字段必须 |
...
| 09 | 风险事件表 + Risk Service | B | 08 | R2 | risk_events 表 + API | Data Product / Prediction Run | prediction_run_id 必须 |
| 10 | 预测批次表 + Prediction Run Service | B | 09 | R2 | prediction_runs 表 + active_run 切换 | Prediction Run | 支持回滚 |
| 11 | L0 Dashboard Data Product | B | 05, 06, 09 | R2 | L0 KPI/TopN API | Perf-SLO | 强缓存/预聚合 |
...
| 21 | L0 Sidebar（省级态势抽屉） | C | 11, 20 | R1 | L0 UI 组件 | Data Product | 3 Tab + KPI + Top5 |
...
| 步骤编号 | 步骤名称 | 泳道 | 依赖 | Reuse Type | 交付对象 | 验收锚点 | 风险点/注意事项 |
|---:|---|---|---|---|---|---|---|
| 30 | 理赔表 + Claim Service | B | 06, 09 | R2 | claims 表 + API | Data Product | 仅历史生成 |
...
| 39 | AI Insight Cards | C + E | 37, 20 | R1 | 洞察卡组件 | Access Mode | 导演式联动 |
...
│       ↓                                                       │
│  [21] L0 Sidebar  [22] Weather Layer  [23] Risk Layer         │
...
**可演示闭环**：
- 打开页面 10s 内看到 L0 态势
- 点击任意区域锁定 → L0 高亮 → 洞察卡刷新
- 点击 "See more" → Panel Half → Timeline 展示
- Ranking Click → Map fly-to + lock
...
- AI Insight Cards + CTA 联动
- AI 聊天窗

**可演示闭环**：
- L2 Details 在不同 Mode 下按规则隐藏/脱敏/全量
- AI Insight Cards 输出结论 + CTA
...
- [ ] Combined/Policies/Claims 三 Tab 的 KPI + Top5 可用（注：Phase 1/2 允许 Claims 为 disabled/placeholder；Phase 3 起必须可用）
- [ ] Ranking Click 能驱动 Map 锁定
- [ ] Timeline 三泳道对齐（Weather/Risk/Claims）（注：Phase 2 允许 Claims lane 为 disabled/placeholder；Phase 3 起必须可用）
- [ ] AI Insight Cards + CTA 能触发 UI 联动
...
| **L1 API + L0 Sidebar** | 步骤 13 与步骤 21 | L0 API 可用 |
