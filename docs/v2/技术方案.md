# Index Insurance 门户网站 - 技术方案文档

## 1. 项目概述

### 1.1 项目背景
基于需求分析文档，本项目需要将现有的 SVG 模拟地图替换为真实的 Google Maps JavaScript API 实现，并集成 Google Contextual View AI kit，打造一个功能完整的 Index Insurance 门户网站。

### 1.2 技术目标
- 集成 Google Maps JavaScript API 实现真实地图渲染
- 实现 4 个数据图层（历史/预测天气数据热力图、历史/预测风险事件）
...
用户操作 (手动/AI)
    ↓
参数变更 (selectedRegion, dataType, dateRange, selectedProduct)
    ↓
...
4. 根据天气数据动态调整颜色深度和透明度
5. 历史数据使用蓝色系，预测数据使用紫色系

**数据结构：**
```typescript
interface WeatherHeatmapData {
  district: string;
...
2. 根据 `dataType` 动态显示可操作的图层
3. 历史数据模式：仅"Historical Weather"可操作
4. 预测数据模式：仅"Predicted Weather"可操作
5. 风险事件图层：仅在选中产品时可用
...
**重要说明**：以下计算策略仅涉及**产品级的风险事件触发条件**，不涉及保单级的赔付原则（如赔付频率限制、理赔额度等）。

**降雨量日内产品计算策略：**
- 数据粒度：小时级数据
- 时间窗口：4 小时滑动窗口（时间区间：00:00 to 23:59）
- 累计方式：包含当前小时和此前连续 3 小时的累计值
- 阈值判断：累计值 > 阈值（多档阈值对应 tier 1, tier 2, tier 3）
- **注意**：仅计算产品级的风险事件触发条件，不考虑保单级的赔付频率限制
...
**重要说明**：当前实现仅涉及**产品级的风险事件触发条件**计算，不涉及保单级的赔付计算。

**产品级的风险事件触发条件**（当前实现）：
- 定义什么情况触发风险事件，触发风险的级别（tier 1, tier 2, tier 3）
- 输入：产品定义、天气数据、时间范围
- 输出：风险事件列表（包含触发时间、区域、级别等）
- 用途：风险可视化、风险分析

**保单级的赔付原则**（未来扩展）：
- 定义赔付多少金额，赔付几次（如"once per day per policy"、"对应不同理赔额度"）
- 输入：风险事件列表、保单配置
- 输出：赔付金额、赔付次数、赔付时间线
- 用途：赔付计算、理赔管理
