...
- **Dashboard Workbench**：地图主舞台 + L0 省级态势侧栏 + Region Intelligence Panel（L1/L2）+ 顶栏全局过滤 + AI Insight Cards（导演式 CTA）
...
- **L0 省级态势（全局背书）**
  - 输出：KPI + Top 排名（Combined / Policies / Claims）
...
- **View State（工作台状态）**
  - region selection（hover/lock）、time range、weather type、product、layer toggles、panel snap（peek/half/full）、tab（combined/policies/claims）
- **Access State（模式与能力）**
...
- **Dashboard Query（L0）**
  - KPI + 排名（Combined/Policies/Claims）
  - 强缓存/预聚合优先
...
### 5.3 L0 排名与地图叠加必须优先预聚合/强缓存

**原则**：

- L0 KPI/Top 排名属于“高频强交互背书”，必须优先强缓存/预聚合
...
- **前端交互事件（示例命名）**：map_hover、map_lock、ranking_click、time_range_change、timeline_brush、weather_type_toggle、layer_toggle、panel_snap_change、ai_cta_click
...
|---|---|---|---|---|---|---|---|
| L0 Dashboard（省级态势） | L0 | “10 秒内一眼看到态势”：KPI + TopN 排名（Combined/Policies/Claims） | region=province、time_range、data_type、weather_type（可选）、access_mode、prediction_run_id（predicted） | 聚合 + 排名 | Demo 需范围化/隐藏敏感口径；Partner 更细；Admin 全量 | 强缓存/预聚合优先；predicted 与 run 绑定失效 | p95 < 500ms（缓存命中）；新鲜度可容忍分钟级 |
...
| Map Click Lock / Ranking Click（重） | L1（必要时预取 L2 摘要） | 锁定后再加载；保持地图主舞台；避免并发风暴 | 选区锁定、联动刷新 |
| “See more” / AI CTA（显式下钻） | L2（按 Mode 裁剪） | 必须 Mode 校验；Demo 默认摘要化 | 打开证据链/定位小节 |
