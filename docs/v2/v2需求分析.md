...
- **智能交互升级**：AI 从“聊天回答”升级为“导演式联动”（Insight Cards + CTA + UI Intent），推动用户进入正确的解释路径而非堆信息。
...
- **Partner（合作伙伴）**：保司/渠道/再保/数据合作方，目标是“更深 KPI/排名/对比，有限下钻”
- **Admin/Internal（内部）**：产品/运营/风控/研发，目标是“全量口径、明细追溯、回放与审计”

### 2.2 使用场景（v2）

#### 场景 1：路演态势播报（默认）
- 打开页面 10 秒内看到省级态势（L0 KPI + Top5 排名）与地图主舞台的天气/风险动效
- 点击任意区县锁定 → 看到该区的“区域情报面板”入口与洞察卡导演提示

#### 场景 2：区域下钻解释（L1）
- Map Click / Ranking Click 锁定区域
- 通过 “See more / Insight CTA” 打开区域情报面板到 Half
...
- Partner：开放更多 KPI/排名与对比能力，明细字段脱敏
...
#### 场景 6：AI 导演式联动（降低门槛）
- AI Insight Cards 输出一句话结论 + CTA（Show on timeline / Overlay thresholds / Compare / Open details）
...
### 3.2 Left Sidebar（L0：省级态势抽屉）
#### 3.2.1 Tabs
- Combined / Policies / Claims 三个 tab

#### 3.2.2 KPI + 排名（Top5）
- 每个 tab 展示 3–5 个 KPI + Top5 排名 +（可选）tiny trend
- 排名点击作为导航：**Ranking Click → Map fly-to + lock region**

#### 3.2.3 AI Insight Cards（导演提示）
- 固定 3 张洞察卡（节流刷新）
- 每张洞察卡包含：结论句 + 证据点（数字）+ 口径（区域/时间/产品/数据类型）+ CTA
- CTA 必须触发 UI 联动（见 5 交互需求），而不是仅文本回答

### 3.3 Map Stage（地图主舞台）
#### 3.3.1 区域交互
- Hover（轻交互）：边界高亮 + mini tooltip（≤3 指标），**不得触发重请求**
- Click（锁定，重交互）：锁定选区、必要时 fly-to、刷新洞察卡；默认不自动展开情报面板
...
|---|---|---|---|---|---|---|
| L0 Dashboard（省级态势） | L0 | “10 秒内一眼看到态势”：KPI + TopN 排名 | 聚合 + 排名 | region_scope=province、data_type、weather_type（可选）、access_mode | Demo 可范围化/隐藏敏感口径；Partner 更细；Admin 全量 | predicted 必须携带 prediction_run_id（或由 active_run 解析） |
...
| AI Insights（洞察卡） | AI | 一句话结论 + 可执行 CTA（导演式联动） | 文本 + UI Intent | 依赖 L0/L1（以及打开 L2 时） | Demo 避免敏感结论与越权 CTA | predicted 必须与页面批次一致（避免解释断裂） |
...
### 5.1 状态机（S0-S3）
以 v2 页面设计的状态机为准：
- S0 初始（L0 展开，Region Panel Collapsed/Peek）
- S1 区域锁定（Map lock，L0 高亮，洞察卡刷新）
- S2 情报面板打开（Half，定位到目标 section）
- S3 分析模式（Full，sticky nav）

### 5.2 轻/重交互边界（避免请求风暴）
- Hover：只做高亮与轻量 tooltip，禁止触发 L1/L2 明细请求
- Click lock / Ranking click：允许触发 L1 最小集（但默认不打断打开面板）
- “See more / Insight CTA”：允许触发 L2（按需加载）

### 5.3 联动矩阵（摘要版）
必须支持以下关键联动：
- Ranking Click → Map fly-to + lock region
- See more / Insight CTA → 打开 Region Panel 并定位到指定 section
...
### 9.1 功能验收
- [ ] L0：Combined/Policies/Claims 三 tab 的 KPI + Top5 排名可用，且 Ranking Click 能驱动 Map 锁定
...
- [ ] AI：Insight Cards + CTA 能触发 UI 联动并定位到解释路径
...
- **Director Cues**：AI 洞察卡 + CTA 驱动 UI 联动
