...
- **L0 省级态势（全局）**：左侧背书与导航（KPI + Top 排名），目标是“10 秒内一眼看到态势”
...
- **事件驱动联动**：Map Hover/Click、Ranking Click、Time Brush、Weather Type Toggle、Layer Toggle、Product Selector、Panel Snap 等触发统一的 UI Event
...
- **Partner（合作伙伴）**：开放更深的排名与 KPI；明细可字段级脱敏
