...
- **渲染形态**：CSR 为主（地图/交互优先）；如需首屏优化再评估 SSR
- **样式**：Tailwind CSS
...
- **数据库**：PostgreSQL 14.7（无 PostGIS/无扩展）
- **缓存**：Redis 7+
- **任务队列**：Celery 5.3+ + Redis
- **ORM**：SQLAlchemy 2.0+（无 PostGIS 依赖）
- **数据验证**：Pydantic 2.0+
...
risk_events (风险事件)
    ├── weather_data (历史天气数据) [间接关联，通过region和timestamp]
...
- **写入策略**：产品写入/修复走内部接口（与 policies/claims/risk_events 的写策略一致，非对外公开）
...
> 本节抽取“仍应沿用”的前端地图集成关键点（不复用旧文档的数据实体字段），并与 v2 Fullstack（Vite + TanStack Query + Zustand）对齐。  
...
- **Key 注入**：仅通过环境变量读取（前端 `VITE_GOOGLE_MAPS_API_KEY`），禁止硬编码
