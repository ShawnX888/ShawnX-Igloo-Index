...
- `weatherData`: `WeatherData[]` (天气原始数据流)
  - **扩展数据支持**：内部使用 `useExtendedWeatherData` 获取扩展时间范围的天气数据，确保时间窗口起始位置的风险事件计算准确

### 2.2 内部逻辑
1. **监听触发**：监听上述所有参数的变更，自动触发计算。
2. **扩展数据获取**：使用 `useExtendedWeatherData` 获取扩展时间范围的天气数据，确保时间窗口起始位置的风险事件计算准确。
3. **服务协同**：内部调用 `RiskCalculationService` 进行逻辑计算（使用扩展数据）。
4. **数据聚合**：
   - **全域统计**：对当前省份内所有城市的风险事件进行**不分级**的数量统计（用于地图标记）。
   - **选中城市统计**：对当前选中城市的风险情况进行**分级**（Tier 1/2/3）的数量统计和详细描述。
5. **结果过滤**：计算完成后，过滤到用户选择的时间窗口（`dateRange`），只保留该时间范围内的风险事件用于展示。

### 2.3 输出结果 (Outputs)
- `riskEvents`: `RiskEvent[]` (原始风险事件列表，已过滤到用户选择的时间窗口)
- `mapMarkersData`: `RiskData[]` (用于地图展示的聚合数据：城市 -> 风险事件总数)
- `detailedStatistics`: `RiskStatistics` (用于数据面板的风险分级统计)
- `isCalculated`: `boolean` (计算状态标识)

**注意**：`riskEvents` 中的风险事件已过滤到用户选择的时间窗口（`dateRange`），确保只包含该时间范围内的事件。
