...
│ - Default: Expanded            │ - Weather Visual Layer (dynamic, 按天气类型) │
│ - Internal Scroll              │ - Risk Layer (events/spot, 按天气类型)       │
│                               │ - Claims Layer (optional overlay, amount/ ColumnLayer or HexagonLayer)            │
│ [AI Insight Cards ★]           │ - Product Selector (浮层，随天气类型更新)      │
│ - 3 cards, 1 line insight      │ - Selection: hover highlight / click lock    │
│ - Each card has CTA → UI action│ - Legend: thresholds / tiers / units         │
│                               │ - "See more" entrypoint (opens Region Panel) │
│                               │                                             │
...
- **天气类型切换（Weather Type Toggle）**：切换天气类型（降雨/风/...）后，地图可视化图层更新；产品选择器自动过滤（只显示支持当前天气类型的产品）；左侧栏 KPI/排名口径可能变化（若按天气类型区分）；区域情报面板的 Timeline 中 Weather/Risk 泳道同步更新。
...
| **Left Sidebar：L0 KPI（省级背书）** | ✅（只给“背书级”KPI，金额可范围化） | ✅（更完整口径） | ✅（全量口径） |
| **Left Sidebar：Top5 排名** | ⚠️ 可选（可隐藏具体金额，只展示相对强弱/区间） | ✅（显示数值） | ✅（显示数值） |
| **AI Insight Cards（洞察卡）** | ✅（强推荐：导演提示） | ✅ | ✅ |
...
- **省级 KPI 行**（存量口径，2 个）：**总保单数**、**总保费**  
  - **统计口径**：保单保期与当前时间区间 **有重叠**（overlap）即计入（避免把“销售”误读为“存量”）
- **排名可视化**：**横向条形图（Leaderboard）** + 数值标签  
  - 交互：Hover 显示区名/值/占比；Click 切换地图选区；支持 Sort（按保单数/保费/覆盖等）
- **不提供趋势图**：避免被误解为“保单销售趋势”（此处定位是存量背书）
...
- **KPI（3 个）**：**理赔件数**、**理赔金额**、**理赔率**（理赔件数 / 保单数）  
  - 说明：基于阈值的自动触发理赔，不提供平均结案时长、拒赔率/通过率等“人工处理”指标，避免口径不一致
- 排名：Top 理赔区（件数/金额双维可切换）
- **趋势（可切换）**：理赔件数 **或** 理赔金额（单指标趋势，避免双轴误读；需要对比时用 toggle 切换）
