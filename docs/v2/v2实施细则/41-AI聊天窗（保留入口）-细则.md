...
AI 聊天窗（Chat Window）：作为 v2 的稳定入口，承接用户自然语言输入、追问解释与兜底交互，并与 Router Agent（Step 37）/AI Insight（Step 39）/Intent Execution（Step 40）通过同一套契约联动
...
- 保留“聊天窗”作为 **永远存在的稳定入口**（RD 中 v1→v2 的关键复用点）：
  ...
  - 与 AI Insight（导演式联动）并存：AI Insight 是主线叙事入口，聊天窗是稳定入口与兜底
...
- 严格对齐三条红线：
 ...
 - 额外强制（Phase 1/2 口径收敛）：
   - 当 Data Products meta 声明 `claims_available=false` 时：聊天窗不得推断/编造 claims 事实，不得输出 claims 结论或 claims 相关 CTA；必须明确告知“理赔事实域尚未上线/当前不可用”，并引导替代解释路径（Timeline/Risk/Weather/Overlays）。

### 非目标

- 不在本细则实现 Router Agent/Expertise Agents（Step 37/38）。
- 不在本细则实现 AI Insight（Step 39）。
...
> SSE 响应建议强制 `Cache-Control: no-store`（避免代理缓存）以避免代理缓存。
...
- 禁止混用证据：
 ...
 - claims 可用性（强制）：
   - 当 `claims_available=false`（Phase 1/2 常态）时，任何 `why`/`details` 类追问不得引导到 claims 事实；只能基于 Weather/Risk 等可用证据做解释，并明确声明 claims 不可用。
...
- 前端可通过 BFF/代理层返回 SSE 流式 Response（Route Handler/Express 等均可）；
- FastAPI 可用 `StreamingResponse(async_generator, media_type="text/event-stream")` 产出 SSE（官方示例）。

> 工程上推荐：前端可选轻量 BFF 统一对外 SSE（路由与鉴权），后端 FastAPI 专注 Agent 逻辑；但两端任一处实现 SSE 都可，只要契约与可观测字段一致。
