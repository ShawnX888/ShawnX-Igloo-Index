...
- 把 AI 从“聊天回答”升级为“导演式联动”（与 `RD-AI聊天窗与洞察联动.md` 对齐）：
  - 输出不仅是文本，还包含 **AI Insight + CTA + UI Intent**
...
- 不在本细则实现前端 Chat UI/AI Insight 组件（Step 39/41/40 承接）。
...
- `insight_cards[]`（AI Insight，导演式联动）：
  - `title`（一句话结论）
...
- `docs/v2/v2复用逻辑摘录/RD-AI聊天窗与洞察联动.md`（聊天窗 + AI Insight + CTA→Orchestration）
...
- [ ] Router Agent 能根据用户输入生成：文本解释 +（可选）AI Insight + 可执行 UI intents。
...
- 收敛：仅输出 AI Insight + 少量安全 intents（open_panel/seek_timeline），禁用 open_details/导出类 intent
