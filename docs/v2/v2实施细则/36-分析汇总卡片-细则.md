...
> 强制口径（Phase 2）：claims 事实域在 Phase 3（Step 30/31/32）。因此在 Phase 2（或任何 `claims_available=false` 的场景）：
> - claims 相关卡片必须为 disabled/N/A（可见但不可用）；
> - 原因必须来自 L1 Data Product 的 meta（禁止前端自行猜测/推断/模拟）；
> - AI（AI Insight/聊天）不得以“正式理赔事实”作结论或证据点。
...
- 与 Step 33 一致：predicted 下无正式 claims 事实时，claims 相关 KPI 必须有明确口径说明（例如只展示 historical claims 或明确显示 N/A；由后端策略决定并在 meta 中说明）
 - 额外强制：当 L1 meta 声明 `claims_available=false` 时（无论 historical/predicted），claims KPI 必须为 N/A/disabled 并解释原因。
...
- [ ] predicted KPI 与页面批次一致（prediction_run_id 不混用），且口径说明清晰。
 - [ ] 当 `claims_available=false`：claims KPI 卡片为 disabled/N/A，且解释来自 L1 meta；不存在“前端自算/前端补齐”的行为。
