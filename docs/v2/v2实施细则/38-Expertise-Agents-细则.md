...
- 不在本细则实现前端 Chat UI / Insight Cards 组件（见 Step 39/41）。
...
> 注意：Expertise Agents 的输出应为 **纯 JSON**（结构化），由 Router Agent 统一转换为可读文本/洞察卡。
...
- `suggested_insight_cards[]`（可选）：候选洞察卡草案（由 Router 决策是否输出）
...
- `docs/v2/v2复用逻辑摘录/RD-AI聊天窗与洞察联动.md`（两条腿：聊天窗 + 洞察卡；动作必须走 Orchestration）
...
- 收敛输出：只输出洞察卡草案，不输出可执行 intents（交由后续版本打开）
