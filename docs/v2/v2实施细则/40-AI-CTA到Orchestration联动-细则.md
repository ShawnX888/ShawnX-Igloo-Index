...
AI CTA → UI Orchestration 联动：将 Router Agent/AI Insight 产生的结构化 UI intents 通过“门控 + 节流 + 可回放”执行到前端状态机与数据产品请求层
...
- 不在本细则实现具体的 AI Insight UI（Step 39）。
...
### 1) Intent 输入（来自 Router/AI Insight）
...
- [ ] 点击 AI Insight CTA 能生成 intent，并通过 Orchestration 执行到正确的 UI 状态与 dp 请求。
