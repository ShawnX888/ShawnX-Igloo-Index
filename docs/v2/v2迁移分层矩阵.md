...
| AI 对话面板（ContextualAssistant） | AI Insight Cards +（可选）聊天入口 | R1 | FE + BE | v2 主线是“导演式联动（CTA+UI Intent）”；聊天可保留为入口，但不再作为唯一输入方式 | AI Insights、Access Mode |
| GPS 定位 + 反向地理编码 | 可选能力（成本/合规敏感） | R1 | FE + BE | v2 需求里定位属于可选；必须做节流/限流/失败降级，并遵守 Google Maps 条款 | Compliance、Perf/SLO |
