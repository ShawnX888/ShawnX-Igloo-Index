...
|---|---|---|
| **架构形态** | 前端 Mock 驱动，无持久化 | 全栈：Vite + FastAPI + PostgreSQL 14.7 + Redis + Celery |
...
**接口策略**：对外仅提供读路径（GET 列表/详情/统计），写入由任务/内部接口完成（不对外公开）。
**内部写入能力（归属）**：
- Policies：内部录入/修复任务
...
┌─────────────────────────────────────────────────────────────┐
│                      数据层 (Data Layer)                      │
│  - PostgreSQL 14.7（无扩展）(products/policies/claims/       │
│    risk_events/prediction_runs/weather_data)                 │
