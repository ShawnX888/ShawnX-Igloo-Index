...
- PostgreSQL 14.7（本地 Docker，不包含 PostGIS 扩展）
- Redis 7+（本地 Docker）
...
2. 编辑 `.env` 文件，确认配置项：
   - `DATABASE_URL` - 本地数据库连接字符串（已自动配置）
   - `GOOGLE_MAPS_API_KEY`
   - `GOOGLE_GENERATIVE_AI_API_KEY`
   - 其他配置项

**数据库配置说明**：
- 数据库使用本地 Docker PostgreSQL 14
- 连接信息：`postgresql+asyncpg://igloo:igloo_dev@localhost:5432/igloo_index`
